...
import datetime
from ktf.env import MAIL_COMMAND,SUBMIT_COMMAND,SCHED_TYPE,DEFAULT_QUEUE,MY_MACHINE,MY_MACHINE_FULL_NAME,CORES_PER_NODE,SACCT_COMMAND,clean_line
...
class engine(object):
    ...
    def initialize_parser(self):
        ...
        self.parser.add_argument("-sl", "--status-long",
                                 action="store_true", help=argparse.SUPPRESS)
        self.parser.add_argument("-sa", "--status-all",
                                 action="store_true", help=argparse.SUPPRESS)
    ...
    def rollback_workflow(self,key='*'):
        ...
        self.log_info('Currently rolling back internal status to get back to step %s ' %
                      step_attempt)
        ...
        if not(self.TASKS[step][task_id]['counted']):
            ...
            if additional_user_checking:
                ...
                if task_id in self.STEPS[step]['global_completion']:
                    for stuff in ['global_completion','global_success','global_failure',
                        ...
                        if not(task_id in self.STEPS[step][stuff]):
                            continue
                        self.STEPS[step]['%s_percent' % stuff] -= 100. / \
                            self.STEPS[step]['global_items']
                        ...
                        if previous_stuff in ["",",",",,"]:
                            ...
                            self.STEPS[step][stuff] = map(
                                lambda x:int(x),previous_stuff[1:-1].split(","))
                ...
                if is_user_checked is True:
                    ...
                    self.STEPS[step]['global_success_percent'] += 100. / \
                        self.STEPS[step]['global_items']
                    ...
                    try:
                        self.STEPS[step]['global_failure'] = list(
                            self.STEPS[step]['global_failure']) + [task_id]
                    except:
                        print("self.STEPS[step]['global_failure'] = %s" %  \
                    ...
                    self.STEPS[step]['global_failure_percent'] += 100. / \
                        self.STEPS[step]['global_items']
...
#     self.log_debug("get_status:beg -> TASKS=\n%s " % pprint.pformat(self.TASKS),3)
        ...
        for step in self.STEPS.keys():
            ...
            self.log_info('get_current_job_status: status of step %s : %s' %
                          (step,status),1, trace='STATUS_DETAIL')
        ...
        if len(jobs_to_check) > 0:
            ...
            # SK???? what if job are done, we don't update their status??????????
            # seems wrong to me
            # if in_queue_only:
            ...
            self.log_debug('jobs_gone=%s' % ",".join(
                map(lambda x: str(x),jobs_gone)),1,trace='STATUS_DETAIL')
                ...
                for task_field in tasks_checked.keys():
                    ...
                    try:
                        ...
                        if status in JOB_POSSIBLE_STATES:
                            self.log_debug('tasks=%s step=%s' %
                                           (tasks,step),2,trace='STATUS_DETAIL')
                            for task in tasks:
                                self.log_debug('status=%s task=%s' %
                                               (status,task),2,trace='STATUS_DETAIL')
                                if status[-1] == '+':
                                    status = status[:-1]
                                if task.find('.') > -1:
                                    continue
...
#      if self.STEPS[step_name]['status'] in ['SUCCESS', 'WAITING','PENDING','FAILURE','ABORTED']:
        ...
        if len(steps_to_check):
            self.log_debug('fixing steps /%s/ ' %
                           pprint.pformat(steps_to_check.keys()), 4, trace='STATUS_DETAIL')
...
#     self.log_info("get_status:end -> TASKS=\n%s " % pprint.pformat(self.TASKS),3)
    ...
    def system(self,cmd,comment="No comment",fake=False,verbosity=1,
        ...
        self.log_debug("\tcurrently executing /%s/ :\n\t\t%s" %
                       (comment,cmd),verbosity,trace='SYSTEM')
    ...
    def send_mail(self,msg='',level=0,subject='',to=None):
        ...
        mail_file = os.path.abspath("%s/mail.txt_%s_%s" %
                                    (self.MAIL_DIR,os.getpid(),int(time.time())))
    ...
    def check_for_pattern(self,pattern,file_mask=None,files=None,return_all=False,col=None):
        ...
        if file_mask:
            ...
            self.log_debug(
                'filtered from mask %s --> files : [%s] ' % (file_mask,",".join(files)),2)
        else:
            files_to_scan = files
            files = []
            ...
            if len(fields) == 2:
                try:
                    ...
                    if expression == '#':
                        ...
                        if (then_present):
                            self.log_debug('IF THEN ELSE found at %s' %
                                           (l), 4, trace='TEMPLATE,IF,LONGIF')
                            fields[1] = fields[1].replace('___THEN___','')
                        else:
                            self.log_debug('one line IF found at %s' %
                                           (l), 4, trace='TEMPLATE,IF,LONGIF')
                        ...
                        self.log_debug('output to add: %s' %
                                       (output_to_add), 4, trace='TEMPLATE,IF,LONGIF')
                        ...
                        output_to_add = l
                        self.log_debug('output to add: %s' %
                                       (output_to_add), 4, trace='TEMPLATE,IF,SHORTIF')
                    cond = fields[1][:-1]
